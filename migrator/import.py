...
# All sections use dynamic #fields: headers from the export file
SECTIONS = {
    "R": [Regime, None],
    "E": [Economy, None],
    "S": [Special, None],
...
async def load(message):
    ...
    for index, line in enumerate(lines, start=1):
        ...
        if index % 10000 == 0:
            output[-1] = f"- Reading migration file... ({index:,}/{len(lines):,})"
            await message.edit(embed=reload_embed())
        ...
        for value, line_data in zip(section_full[1], line.split("╵")):
            ...
            if value not in fields_map:
                # Store unknown v3-only fields, will be dropped later
    ...
    for (item, section_key) in processing_order:
        ...
        for model in rows:
            ...
            # Remove v3-only fields not present in v2
            v3_only = ['extra_data', 'deleted', 'translations', 'moderator_id',
                       'mention_policy', 'friend_policy', 'trade_cooldown_policy']
            for f in v3_only:
                model.pop(f, None)
            ...
            # Map v3 policy enums to v2 (same int values, just verify)
            if item == Player:
...
async def sequence_model(model):
    ...
    try:
        ...
        await client.execute_query(
            f"SELECT setval('{model._meta.db_table}_id_seq', {last_id});"
        )
    except Exception:
        pass
...
async def clear_all_data():
    ...
    try:
        ...
        for m in all_models:
            try:
                await client.execute_query(
                    f"ALTER SEQUENCE {m._meta.db_table}_id_seq RESTART WITH 1;"
                )
            except Exception:
                pass
